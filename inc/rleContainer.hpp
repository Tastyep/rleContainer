...
template <typename T> struct rleData {
  T data;
  unsigned int count;

  rleData(const T &d, unsigned c) : data(d), count(c) {}
};

enum class blockPlacement { Align, Fragment };

template <typename T> class rleContainer {
public:
  rleContainer();
...
template <typename T>
rleContainer<T>::rleContainer() : _last(blockPlacement::Fragment) {}

template <typename T> void rleContainer<T>::push_back(const T &val) {
...
template <typename T> T &rleContainer<T>::operator[](unsigned int idx) {
...
template <typename T> std::size_t rleContainer<T>::memory_used() const {
  std::size_t fragmentSize = 0;

  for (auto vec : _fragmented)
    fragmentSize += vec.size() * sizeof(T);
  return _aligned.size() * sizeof(rleData<T>) + fragmentSize +
         _links.size() * sizeof(unsigned int);
}
